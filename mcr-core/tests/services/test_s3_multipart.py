...
from mcr_meeting.app.infrastructure import s3 as s3_service
...
def test_complete_multipart_upload_success(mock_s3_client):
    ...
    s3_service.complete_multipart_upload_in_s3(req)
...
def test_complete_multipart_upload_failure(mock_s3_client):
    ...
    with pytest.raises(Exception):
        s3_service.complete_multipart_upload_in_s3("test-key", "uploadid", [])


@patch("mcr_meeting.app.infrastructure.s3.s3_client")
def test_abort_multipart_upload_success(mock_s3_client):
    mock_s3_client.abort_multipart_upload.return_value = {}
    s3_service.abort_multipart_upload_in_s3("test-key", "uploadid")
