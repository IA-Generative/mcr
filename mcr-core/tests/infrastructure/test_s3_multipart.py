...
def s3_settings_mock(monkeypatch):
    ...
    monkeypatch.setattr(s3, "s3_settings", DummySettings())
    return DummySettings()


@patch("mcr_meeting.app.infrastructure.s3.s3_client")
def test_create_multipart_upload_success(mock_s3_client, s3_settings_mock):
    ...
    result = s3.create_multipart_upload(123, req)
...
def test_create_multipart_upload_failure(mock_s3_client):
    ...
    with pytest.raises(Exception):
        s3.create_multipart_upload("test-key", "audio/wav")


@patch("mcr_meeting.app.infrastructure.s3.s3_external_client")
def test_get_presigned_url_for_upload_part_success(mock_external_client):
    ...
    url = s3.get_presigned_url_for_upload_part("test-key", "uploadid", 1)
...
def test_get_presigned_url_for_upload_part_failure(mock_external_client):
    ...
    with pytest.raises(Exception):
        s3.get_presigned_url_for_upload_part("test-key", "uploadid", 1)
...
def test_complete_multipart_upload_success(mock_s3_client):
    ...
    s3.complete_multipart_upload_in_s3(req)
