...
import mcr_gateway.setup.sentry as sentry_module
from mcr_gateway.setup.logger import _json_sink, _serialize_record, setup_logging
...
def test_serialize_record_emits_expected_fields(
    ...
    assert payload["service"] == "mcr-gateway"
...
def test_json_sink_reads_trace_ids_from_owner_and_writes_one_line(
    ...
    monkeypatch.setattr(sentry_module, "current_trace_ids", lambda: ("t-1", "s-1"))
...
def test_current_trace_ids_parses_traceparent(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(
        sentry_module.sentry_sdk, "get_traceparent", lambda: "abc123-def456-1"
    )

    assert sentry_module.current_trace_ids() == ("abc123", "def456")


def test_current_trace_ids_none_without_active_trace(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(sentry_module.sentry_sdk, "get_traceparent", lambda: None)
