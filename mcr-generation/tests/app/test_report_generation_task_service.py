...
class TestGenerateReportFromDocx:
    ...
    def test_no_notes_does_not_invoke_extractor(
        self,
        decision_record: DecisionRecord,
        mock_get_file_from_s3: MagicMock,
        mock_chunk_docx_to_document_list: MagicMock,
        ...
        """notes_content=None must not instantiate or run NotesExtractor."""
        mock_get_file_from_s3.return_value = b"docx"
        mock_chunk_docx_to_document_list.return_value = []
        mock_get_generator.return_value.generate.return_value = decision_record
        mock_extractor_cls = MagicMock()
        monkeypatch.setattr(
            "mcr_generation.app.services.report_generation_task_service.NotesExtractor",
            mock_extractor_cls,
        )

        generate_report_from_docx(1, "transcription.docx", notes_content=None)

        mock_extractor_cls.assert_not_called()

    def test_blank_notes_does_not_invoke_extractor(
        self,
        decision_record: DecisionRecord,
        mock_get_file_from_s3: MagicMock,
        mock_chunk_docx_to_document_list: MagicMock,
        ...
        """notes_content='   ' (whitespace) must not invoke NotesExtractor."""
        mock_get_file_from_s3.return_value = b"docx"
        mock_chunk_docx_to_document_list.return_value = []
        mock_get_generator.return_value.generate.return_value = decision_record
        mock_extractor_cls = MagicMock()
        ...
        generate_report_from_docx(1, "transcription.docx", notes_content="   ")

        mock_extractor_cls.assert_not_called()

    def test_with_notes_invokes_extractor_via_asyncio_run(
        self,
        decision_record: DecisionRecord,
        mock_get_file_from_s3: MagicMock,
        mock_chunk_docx_to_document_list: MagicMock,
        ...
        """notes_content='...' triggers asyncio.run(NotesExtractor().extract_all(...))."""
        mock_get_file_from_s3.return_value = b"docx"
        mock_chunk_docx_to_document_list.return_value = []
        mock_get_generator.return_value.generate.return_value = decision_record

        mock_instance = MagicMock()
        mock_coro = MagicMock()  # placeholder for the coroutine object
        mock_instance.extract_all.return_value = mock_coro
        mock_extractor_cls = MagicMock(return_value=mock_instance)
        ...
        monkeypatch.setattr(
            "mcr_generation.app.services.report_generation_task_service.asyncio.run",
            mock_asyncio_run,
        )

        generate_report_from_docx(1, "transcription.docx", notes_content="some notes")
