...
```bash
VITE_IS_MOCKED=true # active le mode mock (aucune de communication avec le backend)
VITE_API_BASE_URL=http://localhost:5173/api # Url à remplacer par celle du backend lorsque VITE_IS_MOCKED=false
```
...
```sh
docker build -f docker/Dockerfile -t mcr-frontend --build-arg VITE_API_BASE_URL=http://localhost:8000/api .
```

## Lancer le container

```sh
docker run -p 8080:80 --name=mcr-frontend mcr-frontend
```
